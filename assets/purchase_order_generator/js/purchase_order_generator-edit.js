...
function openDatabaseEdit() {
    return new Promise((resolve, reject) => {
        const requestEdit = indexedDB.open('PurchaseOrderDownloadHistoryDB', 2);

        requestEdit.onupgradeneeded = (event) => {
            const dbEdit = event.target.result;
            if (!dbEdit.objectStoreNames.contains('purchaseOrderdownloadHistory')) {
                dbEdit.createObjectStore('purchaseOrderdownloadHistory', { keyPath: 'id', autoIncrement: true });
            }
        };
...
async function loadInvoice() {
    ...
    if (id) {
        try {
            ...
            const transaction = dbEdit.transaction(['purchaseOrderdownloadHistory'], 'readonly');
                    ...
                    setTimeout(async () => {
                        ...
                        savedData.forEach(row => {
                            const newRow = document.createElement('tr');
                                ...
                                newRow.innerHTML = `
                                <td><input type="text" class="form-control sku-input" data-i18n-placeholder="purchase.table_placeholder_sku"  placeholder="Enter Product Sku..."  value="${row.sku || ''}"><div class="suggestion-box"></div></td>
                                <td><input type="text" class="form-control name-input" data-i18n-placeholder="purchase.table_placeholder_description_item"  placeholder="Description of item/service..." value="${row.description || ''}"><div class="suggestion-box"></div></td>
                                <td><input class="form-control quantity-input" data-i18n-placeholder="purchase.table_placeholder_quantity" type="number" value="${row.value1 || 0}" oninput="calculateAmount(this)" placeholder='Quantity'></td>
                                <td><input class="form-control rate-input" type="number" data-i18n-placeholder="purchase.table_placeholder_rate" value="${row.value2 || 0}" oninput="calculateAmount(this)" placeholder='Rate'></td>
                        ...
                        // document.getElementById('balance-due').textContent = entry.balanceDue || '';
