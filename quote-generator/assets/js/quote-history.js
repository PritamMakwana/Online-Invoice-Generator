...
async function loadDownloadHistory() {
    try {
        ...
        const tableBody = document.querySelector('#download-history-table-quote tbody');
        tableBody.innerHTML = '';
        ...
        downloadHistory.forEach(entry => {
            ...
            statusBadge.setAttribute('data-i18n', 'quote_history.td_status_created');
            ...
            downloadOptionButton.setAttribute('data-i18n', 'quote_history.td_option_download');
            ...
            editOptionButton.setAttribute('data-i18n', 'quote_history.td_option_edit');
            editOptionButton.textContent = 'Edit';
            editOptionButton.addEventListener('click', () => {
                window.location.href = `/quote-generator.html?id=${entry.id}`;
            });
            ...
            deleteOptionButton.setAttribute('data-i18n', 'quote_history.td_option_delete');
            deleteOptionButton.textContent = 'Delete';
            deleteOptionButton.addEventListener('click', async () => {
                // if (confirm('Are you sure you want to delete this quote enerator?')) {
        ...
        if ($.fn.DataTable.isDataTable('#download-history-table-quote')) {
            $('#download-history-table-quote').DataTable().clear().destroy();
        }
        DatatableLanguage('#download-history-table-quote');
...
function openDatabase() {
    return new Promise((resolve, reject) => {
        const request = indexedDB.open('QuoteDownloadHistoryDB', 2);

        request.onupgradeneeded = (event) => {
            const db = event.target.result;
            if (!db.objectStoreNames.contains('quoteDownloadHistory')) {
                db.createObjectStore('quoteDownloadHistory', { keyPath: 'id', autoIncrement: true });
            }
        };
...
function getAllFromDatabase() {
    return new Promise(async (resolve, reject) => {
        ...
        const transaction = db.transaction('quoteDownloadHistory', 'readonly');
...
async function clearAllHistory() {
    try {
        const db = await openDatabase();
        const transaction = db.transaction('quoteDownloadHistory', 'readwrite');
        const store = transaction.objectStore('quoteDownloadHistory');
        const clearRequest = store.clear();

        clearRequest.onsuccess = () => {
            window.location.href = 'quote-generator-history.html'
        };
...
async function exportDataToCSV() {
    const db = await openDatabase();
    const transaction = db.transaction(['quoteDownloadHistory'], 'readonly');
    const objectStore = transaction.objectStore('quoteDownloadHistory');
    ...
    request.onsuccess = async (event) => {
        ...
        rows.push(['No', 'PDF Heading', 'From To', 'Bill To', 'Total', 'Date', 'Valid Till Date']);
