...
function templateChoose() {
    ...
    tableRows.forEach(row => {
        ...
        rowObject.sku = row.querySelector('td:nth-child(1) input')?.value || '';
        rowObject.description = row.querySelector('td:nth-child(2) input')?.value || '';
        rowObject.value1 = parseFloat(row.querySelector('td:nth-child(3) input')?.value) || 0;
        rowObject.value2 = parseFloat(row.querySelector('td:nth-child(4) input')?.value) || 0;
        rowObject.amount = parseFloat(row.querySelector('td:nth-child(5)')?.textContent.replace(currency, '').trim()) || 0;
...
let productDb;
const productRequest = indexedDB.open("ProductDatabase", 2);

productRequest.onupgradeneeded = (event) => {
    productDb = event.target.result;
    const objectStore = productDb.createObjectStore("products", { keyPath: "id", autoIncrement: true });
};
...
function searchProductSuggestions(input, fieldType) {
    const value = input.value.trim().toLowerCase();
    if (!value) return; // Exit if input is empty

    const transaction = productDb.transaction(["products"], "readonly");
    const objectStore = transaction.objectStore("products");
    const productRequest = objectStore.getAll();
    ...
    productRequest.onsuccess = async (event) => {
        ...
        for (const product of products) {
            ...
            if (
                (fieldType === "sku" && decryptedProduct.sku.toLowerCase().includes(value)) ||
                (fieldType === "name" && decryptedProduct.name.toLowerCase().includes(value))
            ) {
                matches.push(decryptedProduct);
            }
        }
        ...
        matches.forEach((match) => {
            ...
            option.textContent = `${match.sku} - ${match.name} - $${match.price}`;
            option.onclick = () => fillProductDetails(input, match);
...
// Fill product details on selection
function fillProductDetails(input, product) {
    const row = input.closest("tr");
    // row.querySelector("input[placeholder='Enter Product Sku...']").value = product.sku;
    // row.querySelector("input[placeholder='Description of item/service...']").value = product.name;
    // row.querySelector("input[placeholder='Quantity']").value = 0;
    // row.querySelector("input[placeholder='Rate']").value = product.price;
...
// Add event listener for SKU or product name fields
document.addEventListener("input", (event) => {
    // if (event.target.matches("input[placeholder='Enter Product Sku...']")) {
    //     searchProductSuggestions(event.target, "sku");
    // } else if (event.target.matches("input[placeholder='Description of item/service...']")) {
    //     searchProductSuggestions(event.target, "name");
    // }

    if (event.target.classList.contains("sku-input")) {
        searchProductSuggestions(event.target, "sku");
    } else if (event.target.classList.contains("name-input")) {
        searchProductSuggestions(event.target, "name");
    }
});
