...
function openDatabaseEdit() {
    return new Promise((resolve, reject) => {
        const requestEdit = indexedDB.open('DownloadHistoryDB', 1);

        requestEdit.onupgradeneeded = (event) => {
            const dbEdit = event.target.result;
            if (!dbEdit.objectStoreNames.contains('downloadHistory')) {
                dbEdit.createObjectStore('downloadHistory', { keyPath: 'id', autoIncrement: true });
            }
        };
...
async function loadInvoice() {
    ...
    if (id) {
        try {
            ...
            const transaction = dbEdit.transaction(['downloadHistory'], 'readonly');
            ...
            requestEdit.onsuccess = async (event) => {
                ...
                if (entry) {
                    ...
                    setTimeout(async () => {
                        ...
                        document.getElementById('terms').value = entry.termsSelect || '';
